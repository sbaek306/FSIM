...
! the bandwidth hx for kernel estimator regress
     ...
     hxl=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*3.5
     hxe=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*0.25
...
open(1,file='simu2.txt') ! y m=1.,v=1.
...
subroutine gendata2(n,p1,p2,pt1,pt2,beta1,beta2,q,t,bf,alpha,x,y,z) 
  ...
  double precision :: a,b,a1,a2,b1,b2,a3,a4,a5,b3,b4,b5,dinvnr,integral,m,zb
  ...
  do i=1,n
     do h=1,q+1
        ...
        b8=1.-a8
        b9=1.-a9 
        x(i,h,1)=a1!dinvnr(a1,b1)*sqrt(1./2.)
        x(i,h,2)=a2!dinvnr(a2,b2)*sqrt(1./2.)
        x(i,h,3)=a3!dinvnr(a3,b3)*sqrt(1./2.)
        x(i,h,4)=a4!dinvnr(a4,b4)*sqrt(1./2.)!(t(h)-0.5)**2+rand(0) !(t(h)-0.5)**3+cos(2.*pi*t(h))+dinvnr(a2,b2)
        x(i,h,5)=a5!dinvnr(a5,b5)*sqrt(1./2.)
        x(i,h,6)=a6!dinvnr(a6,b6)
        x(i,h,7)=a7!dinvnr(a7,b7)
        x(i,h,8)=a8!dinvnr(a8,b8)
        x(i,h,9)=a9!dinvnr(a9,b9)
    end do 
...
!  do i=1,n   
  ...
  !  tzb(i)=integral
    ...
    a=rand(0)
    b=1.0-a
    y(i) = sin(2.*m)+log(1.+m**2.)-3. + dinvnr(a,b)*sqrt(0.5*(1.+m**2.)**(1./5.))
  end do 
  return
...
subroutine truedlogeta(n,t,y,f)
  ...
  !u tc=t(:,1,1)
  m=sin(2.*t)+log(1.+t**2.)-3. ! mean 
  v=0.5*(1.+t**2.)**(1./5.)
!  v=0.1*log(1.+t**2.)
!  v=1.0   ! variance
!  v=1
  dm=2.*cos(2.*t)+2.*t/(1.+t**2.)
  dv=0.1*(1.+t**2.)**(-4./5.)*2.*t
!  dv=0.0
!  dv=0.2*t/(1+t**2.)
  t1=(y-m)/v
  t2=t1**2./2.-0.5/v
