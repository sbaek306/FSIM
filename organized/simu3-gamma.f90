...
! the bandwidth hx for kernel estimator regress
     hx=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*0.8
     hxl=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*1.0
     hxe=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*1.0
...
!     hy=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.25
     hy=(1.0*n)**(-1./6.)*sqrt(sum(betat**2.+1.))*0.3
     ! the bandwidth b
!     b=(1.0*n)**(-1./8.)*sqrt(sum(betat**2.+1.))*0.25
     b=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.3
! initialize random number generator at iseed
  tmp=rand(iseed)
  iters=1

open(1,file='simu3-gamma.txt') ! y m=1.,v=1.
