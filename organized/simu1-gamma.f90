...
! the bandwidth hx for kernel estimator regress
     hx=2.5!(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*1.5
     hxl=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*0.5
     hxe=1.5!(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*1.
...
!     hy=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.25
     hy=0.015!(1.0*n)**(-1./6.)*sqrt(sum(betat**2.+1.))*0.75
! the bandwidth b
!     b=(1.0*n)**(-1./8.)*sqrt(sum(betat**2.+1.))*0.25
     b=0.01!(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.05
! initialize random number generator at iseed
  tmp=rand(iseed)
  iters=1

open(1,file='1.txt') ! y m=1.,v=1.
...
subroutine gendata1(p2,pt2,q,t,bf,alpha,betat2) 
  ...
  innerknots = [0.25,0.5,0.75]![0.33, 0.67]
