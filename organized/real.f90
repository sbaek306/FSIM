...
module commondata
  implicit none
  save
  integer,parameter :: n=400,p1=4,m1=3,d=3,q=364
...
! Local estimator 
  betat1=[-0.3,-1.,-1.8]
  betat2=[-0.7,-1.17,3.86,0.65,-0.46,-0.2]!-0.68]

! obtain spline coefficient estimates - betat2 - using LS
  call genbspline(p2,pt2,q,t,bf) 
  betat(1:pt1)=betat1
...
! the bandwidth hx for kernel estimator regress
     ...
     hxl=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*2.0
     hxe=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*0.75
...
!     hy=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.25
     hy=(1.0*n)**(-1./6.)*sqrt(sum(betat**2.+1.))*0.35
! the bandwidth b
!     b=(1.0*n)**(-1./8.)*sqrt(sum(betat**2.+1.))*0.25
     b=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.09
! initialize random number generator at iseed
  tmp=rand(iseed)
  
  open(1,file='real.txt') ! y m=1.,v=1.
!  write(1,*) simus,num,p1,p2,pt,n,zeros(1:pt-8),m1,d 
!  write(1,*) hx,hl,hy,b,zeros(1:pt-5)
  write(1,*) betat
  call getdatax(n,q,p1,x)
  call getdatay(n,y)
  call getz(n,p1,p2,pt1,pt2,beta1,beta2,q,bf,x,z) 
!  print *,z(1,:,:)
...
subroutine getdatax(n,q,p1,x)
  integer,intent(in) :: n,q,p1
  double precision,dimension(n,q+1,p1),intent(out) :: x
  integer(2) :: i,j,k
  open(11,file='x1.dat')
  do i=1,n
     do j=1,q+1
        read(11,*) (x(i,j,k),k=1,4)
     end do
  end do
  close(11)
  return
end subroutine getdatax

subroutine getdatay(n,y)
  integer,intent(in) :: n
  double precision,dimension(n),intent(out) :: y
  open(11,file='cvd1.dat')
  read(11,*) y 
  close(11)
!  y=y*100.
  return
end subroutine getdatay

subroutine getz(n,p1,p2,pt1,pt2,beta1,beta2,q,bf,x,z) 
  use commondata, only: m1,d,pi   
  integer,intent(in) :: n,p1,pt1,p2,pt2,q 
  double precision,dimension(n,p1,p2),intent(out) :: z
  double precision,dimension(n,q+1,p1), intent(in) :: x
...
subroutine genbspline(p2,pt2,q,t,bf) 
  use commondata, only: m1,d,pi   
...
subroutine dlogeta(n,t,y,f)
  ...
  double precision :: eps,z0,hx2=.25
  integer :: i,one
  one=1
  eps=1e-8
  do i=1,n
