...
module commondata
  implicit none
  save
  integer,parameter :: simus=1000,n=500,p1=1,m1=3,d=3,q=100,j1=4
...
! the bandwidth hx for kernel estimator regress
     hx=2.5!(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*2.5
     hxl=(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*0.5
     hxe=1.5!(1.0*n)**(-1./5.)*sqrt(sum(betat**2.+1.))*1.5
...
!     hy=(1.0*n)**(-1./7.)*sqrt(sum(betat**2.+1.))*0.25
     hy=0.03!(1.0*n)**(-1./6.)*sqrt(sum(betat**2.+1.))*2.5
! the bandwidth b
...
! initialize random number generator at iseed
  ...
  open(1,file='simu3c.txt') ! y m=1.,v=1.
...
subroutine gendata2(n,p1,p2,pt1,pt2,beta1,beta2,q,t,bf,betaft,x,y,z)  
  ...
  double precision :: a,b,a1,a2,b1,b2,a3,a4,a5,b3,b4,b5,integral,zb,mean 
...
!  do i=1,n
    ...
    a=rand(0)
    b=1.0-a
    mean =  0.075 + (sum(tmp)+1.23)/1.79*0.95 
